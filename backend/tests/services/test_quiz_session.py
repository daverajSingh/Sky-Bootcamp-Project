from application.services.question import get_quiz_sessions, add_quiz_session, delete_quiz_session, update_quiz_session
import pymysql
import pytest
...
def test_get_quiz_sessions(mocker):
    ...
    mock_instance.query.return_value = [
        {'session_id': 1, 'start_time': '2023-10-01 10:00:00', 'end_time':'2023-10-01 10:30:00'},
        {'session_id': 2, 'start_time': '2023-10-02 11:00:00', 'end_time':'2023-10-02 11:45:00'}
    ]

    quiz_sessions = get_quiz_sessions()
    assert len(quiz_sessions) == 2
    assert quiz_sessions[0]['time_diff'] == '00:30:00'
    assert quiz_sessions[1]['time_diff'] == '00:45:00'
...
def test_add_quiz_session(mocker):
    ...
    mock_instance.execute.assert_called_once_with('INSERT INTO quiz_session (start_time, end_time) VALUES (%s),(%s)')
...
def test_update_quiz_session(mocker):
    ...
    mock_instance.execute.assert_called_once_with("UPDATE quiz_session SET start_time = (%s), end_time = (%s) WHERE session_id = (%s)", (1,'2025-10-27 12:00:00','2025-10-27 12:00:01'))
...
def test_quiz_session_db_errors(mocker):
    ...
    with pytest.raises(RuntimeError) as exc_add:
        add_quiz_session(1,'2025-10-27 12:00:00','2025-10-27 12:00:01')

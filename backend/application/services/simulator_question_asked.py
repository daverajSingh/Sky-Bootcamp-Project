...
def get_simulator_question_asked():
    db = DataAccess()
    try :
        questions_asked = db.query("SELECT question_asked_id, topic_id FROM simulator_question_asked")
...
def get_simulator_question_asked_by_topic_id(topic_id):
    ...
    try :
        questions_asked = db.query("SELECT question_asked_id context FROM simulator_question_asked WHERE topic_id = (%s)", topic_id)
        return questions_asked
    except pymysql.MySQLError as e:
        raise RuntimeError(f'Database query error: {e}')
...
def update_simulator_question_asked(question_asked_id, topic_id):
    db = DataAccess()
    try :
        db.execute("UPDATE simulator_question_asked SET topic_id = (%s) WHERE question_asked_id = (%s)", (topic_id, question_asked_id))
    except pymysql.MySQLError as e:
        raise RuntimeError(f'Database query error: {e}')

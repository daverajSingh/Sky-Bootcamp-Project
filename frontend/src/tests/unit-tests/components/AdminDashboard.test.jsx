...
describe("AdminDashboard", () => {
  ...
  it("renders header and stats correctly", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText("Sky Early Careers")).toBeInTheDocument();
    });
    expect(screen.getByText("Sky Immersion – Admin Page")).toBeVisible();
  ...
  it("displays topic buttons and default Agile questions", async () => {
    render(<AdminDashboard />);
    await waitFor(() => expect(screen.getByText("Agile")).toBeInTheDocument());
    expect(screen.getByText("Agile question 1")).toBeInTheDocument();
    expect(screen.getByText("Agile question 2")).toBeInTheDocument();
  });
  ...
  it("changes questions when topic button is clicked", async () => {
    ...
    expect(screen.getByText("Values question 1")).toBeInTheDocument();
  });

  it("allows selecting up to 4 questions per topic", async () => {
    render(<AdminDashboard />);
  ...
  it("adds a new question to the selected topic", async () => {
    render(<AdminDashboard />);
    await waitFor(() => expect(screen.getByPlaceholderText("Type your question...")).toBeInTheDocument());
  ...
  it("renders quiz results and simulation questions correctly", async () => {
    ...
    expect(screen.getByText("Simulation Questions Asked")).toBeInTheDocument();
  });
});